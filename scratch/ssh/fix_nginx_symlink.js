...
conn.on('ready', () => {
  console.log('SSH Connected - Fixing Nginx symlinks and applying config...');
  conn.exec(`
...
server {
    ...
    # 1. Uploaded products directory (Priority over regex)
    location ^~ /assets/products/ {
        root /var/www/soley/public;
        expires 1M;
    ...
    ssl_certificate /etc/letsencrypt/live/soley.ma/fullchain.pem; # managed by Certbot
    ssl_certificate_key /etc/letsencrypt/live/soley.ma/privkey.pem; # managed by Certbot
...
ln -s /etc/nginx/sites-available/soley /etc/nginx/sites-enabled/soley
